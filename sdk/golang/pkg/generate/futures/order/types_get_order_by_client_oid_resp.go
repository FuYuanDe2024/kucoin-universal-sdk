// Code generated by Kucoin Universal SDK Generator; DO NOT EDIT.

package order

import (
	"github.com/FuYuanDe2024/kucoin-universal-sdk/sdk/golang/pkg/types"
)

// GetOrderByClientOidResp struct for GetOrderByClientOidResp
type GetOrderByClientOidResp struct {
	// common response
	CommonResponse *types.RestResponse
	// Order ID
	Id string `json:"id,omitempty"`
	// Symbol of the contract, Please refer to [Get Symbol endpoint: symbol](https://www.kucoin.com/docs-new/api-3470220)
	Symbol string `json:"symbol,omitempty"`
	// Order type, market order or limit order
	Type string `json:"type,omitempty"`
	// Transaction side
	Side string `json:"side,omitempty"`
	// Order price
	Price string `json:"price,omitempty"`
	// Order quantity
	Size int32 `json:"size,omitempty"`
	// Order value
	Value string `json:"value,omitempty"`
	// Executed size of funds
	DealValue string `json:"dealValue,omitempty"`
	// Executed quantity
	DealSize int32 `json:"dealSize,omitempty"`
	// [Self Trade Prevention](https://www.kucoin.com/docs-new/doc-338146) is divided into these strategies: CN, CO, CB. Not supported DC at the moment.
	Stp string `json:"stp,omitempty"`
	// Stop order type (stop limit or stop market)
	Stop string `json:"stop,omitempty"`
	// Trigger price type of stop orders
	StopPriceType string `json:"stopPriceType,omitempty"`
	// Mark to show whether the stop order is triggered
	StopTriggered bool `json:"stopTriggered,omitempty"`
	// Trigger price of stop orders
	StopPrice float32 `json:"stopPrice,omitempty"`
	// Time in force policy type
	TimeInForce string `json:"timeInForce,omitempty"`
	// Mark of post only
	PostOnly bool `json:"postOnly,omitempty"`
	// Mark of the hidden order
	Hidden bool `json:"hidden,omitempty"`
	// Mark of the iceberg order
	Iceberg bool `json:"iceberg,omitempty"`
	// Leverage of the order
	Leverage string `json:"leverage,omitempty"`
	// A mark to forcely hold the funds for an order
	ForceHold bool `json:"forceHold,omitempty"`
	// A mark to close the position
	CloseOrder bool `json:"closeOrder,omitempty"`
	// Visible size of the iceberg order
	VisibleSize int32 `json:"visibleSize,omitempty"`
	// Unique order id created by users to identify their orders
	ClientOid string `json:"clientOid,omitempty"`
	// Remark
	Remark string `json:"remark,omitempty"`
	// tag order source
	Tags string `json:"tags,omitempty"`
	// Mark of the active orders
	IsActive bool `json:"isActive,omitempty"`
	// Mark of the canceled orders
	CancelExist bool `json:"cancelExist,omitempty"`
	// Time the order created
	CreatedAt int64 `json:"createdAt,omitempty"`
	// last update time
	UpdatedAt int64 `json:"updatedAt,omitempty"`
	// Order Endtime
	EndAt int64 `json:"endAt,omitempty"`
	// Order create time in nanosecond
	OrderTime int64 `json:"orderTime,omitempty"`
	// settlement currency
	SettleCurrency string `json:"settleCurrency,omitempty"`
	// Margin mode: ISOLATED (isolated), CROSS (cross margin).
	MarginMode string `json:"marginMode,omitempty"`
	// Average transaction price, forward contract average transaction price = sum (transaction value) / sum (transaction quantity), reverse contract average transaction price = sum (transaction quantity) / sum (transaction value). Transaction quantity = lots * multiplier
	AvgDealPrice string `json:"avgDealPrice,omitempty"`
	// Value of the executed orders
	FilledSize int32 `json:"filledSize,omitempty"`
	// Executed order quantity
	FilledValue string `json:"filledValue,omitempty"`
	// order status: “open” or “done”
	Status string `json:"status,omitempty"`
	// A mark to reduce the position size only
	ReduceOnly bool `json:"reduceOnly,omitempty"`
}

// NewGetOrderByClientOidResp instantiates a new GetOrderByClientOidResp object
// This constructor will assign default values to properties that have it defined
func NewGetOrderByClientOidResp(id string, symbol string, Type_ string, side string, price string, size int32, value string, dealValue string, dealSize int32, stp string, stop string, stopPriceType string, stopTriggered bool, stopPrice float32, timeInForce string, postOnly bool, hidden bool, iceberg bool, leverage string, forceHold bool, closeOrder bool, visibleSize int32, clientOid string, remark string, tags string, isActive bool, cancelExist bool, createdAt int64, updatedAt int64, endAt int64, orderTime int64, settleCurrency string, marginMode string, avgDealPrice string, filledSize int32, filledValue string, status string, reduceOnly bool) *GetOrderByClientOidResp {
	this := GetOrderByClientOidResp{}
	this.Id = id
	this.Symbol = symbol
	this.Type = Type_
	this.Side = side
	this.Price = price
	this.Size = size
	this.Value = value
	this.DealValue = dealValue
	this.DealSize = dealSize
	this.Stp = stp
	this.Stop = stop
	this.StopPriceType = stopPriceType
	this.StopTriggered = stopTriggered
	this.StopPrice = stopPrice
	this.TimeInForce = timeInForce
	this.PostOnly = postOnly
	this.Hidden = hidden
	this.Iceberg = iceberg
	this.Leverage = leverage
	this.ForceHold = forceHold
	this.CloseOrder = closeOrder
	this.VisibleSize = visibleSize
	this.ClientOid = clientOid
	this.Remark = remark
	this.Tags = tags
	this.IsActive = isActive
	this.CancelExist = cancelExist
	this.CreatedAt = createdAt
	this.UpdatedAt = updatedAt
	this.EndAt = endAt
	this.OrderTime = orderTime
	this.SettleCurrency = settleCurrency
	this.MarginMode = marginMode
	this.AvgDealPrice = avgDealPrice
	this.FilledSize = filledSize
	this.FilledValue = filledValue
	this.Status = status
	this.ReduceOnly = reduceOnly
	return &this
}

// NewGetOrderByClientOidRespWithDefaults instantiates a new GetOrderByClientOidResp object
// This constructor will only assign default values to properties that have it defined,
func NewGetOrderByClientOidRespWithDefaults() *GetOrderByClientOidResp {
	this := GetOrderByClientOidResp{}
	return &this
}

func (o *GetOrderByClientOidResp) ToMap() map[string]interface{} {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["symbol"] = o.Symbol
	toSerialize["type"] = o.Type
	toSerialize["side"] = o.Side
	toSerialize["price"] = o.Price
	toSerialize["size"] = o.Size
	toSerialize["value"] = o.Value
	toSerialize["dealValue"] = o.DealValue
	toSerialize["dealSize"] = o.DealSize
	toSerialize["stp"] = o.Stp
	toSerialize["stop"] = o.Stop
	toSerialize["stopPriceType"] = o.StopPriceType
	toSerialize["stopTriggered"] = o.StopTriggered
	toSerialize["stopPrice"] = o.StopPrice
	toSerialize["timeInForce"] = o.TimeInForce
	toSerialize["postOnly"] = o.PostOnly
	toSerialize["hidden"] = o.Hidden
	toSerialize["iceberg"] = o.Iceberg
	toSerialize["leverage"] = o.Leverage
	toSerialize["forceHold"] = o.ForceHold
	toSerialize["closeOrder"] = o.CloseOrder
	toSerialize["visibleSize"] = o.VisibleSize
	toSerialize["clientOid"] = o.ClientOid
	toSerialize["remark"] = o.Remark
	toSerialize["tags"] = o.Tags
	toSerialize["isActive"] = o.IsActive
	toSerialize["cancelExist"] = o.CancelExist
	toSerialize["createdAt"] = o.CreatedAt
	toSerialize["updatedAt"] = o.UpdatedAt
	toSerialize["endAt"] = o.EndAt
	toSerialize["orderTime"] = o.OrderTime
	toSerialize["settleCurrency"] = o.SettleCurrency
	toSerialize["marginMode"] = o.MarginMode
	toSerialize["avgDealPrice"] = o.AvgDealPrice
	toSerialize["filledSize"] = o.FilledSize
	toSerialize["filledValue"] = o.FilledValue
	toSerialize["status"] = o.Status
	toSerialize["reduceOnly"] = o.ReduceOnly
	return toSerialize
}

func (o *GetOrderByClientOidResp) SetCommonResponse(response *types.RestResponse) {
	o.CommonResponse = response
}
